    ...
    **kwargs,
    ...
    if len(info) == 0:
        await context.bot.send_message(
            ...
            text="Вы не предоставили никакой информации.",
        )
        return
    ...
    if message_link is None:
        message_link = ""
        title = "Личное сообщение в TG"
    else:
        title = "Сообщение в группе TG"
    ...
    **kwargs,
    ...
    if len(context.args) != 1:
        await context.bot.send_message(
            ...
            text=(
                ...
                "Ожидался только id документа, который нужно удалить."
            ),
        )
        return
    ...
    if success:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            reply_to_message_id=update.effective_message.id,
            text="Информация удалена из базы знаний.",
        )
    else:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            reply_to_message_id=update.effective_message.id,
            text=(
                "Что-то пошло не так. Информация не удалена из базы знаний. "
    ...
    **kwargs,
    ...
    if len(context.args) != 1:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            reply_to_message_id=update.effective_message.id,
            text="Вы должны предоставить ссылку!",
        )
        return
...
async def get_user_id_from_message(
    ...
    if banned_user_id is None:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            reply_to_message_id=update.effective_message.id,
            text="Неправильный формат аргументов. Пожалуйста, укажите id "
            "пользователя или напишите эту команду в ответ на его сообщение.",
        )
...
async def ban_user(
    ...
    if banned_admin is not None:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            reply_to_message_id=update.effective_message.id,
            text="Невозможно забанить администраторов.",
        )
        return
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        reply_to_message_id=update.effective_message.id,
        text="Указанный пользователь забанен!",
    )
...
async def unban_user(
    ...
    if result.rowcount > 0:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            reply_to_message_id=update.effective_message.id,
            text="Указанный пользователь разбанен!",
        )
...
async def add_admin(
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        reply_to_message_id=update.effective_message.id,
        text="Указанный пользователь добавлен в список администраторов!",
    )
