...
MEMORY_UPDATE_PROMPT = ChatPromptTemplate.from_messages([
    ("system", """Ты — система обновления памяти бота-проводника по поступлению в Австрию.
...
{{"journey_state": {{"documents_prep": "pending|discussed", ...}}, "summary": "обновлённый текст с ключевыми фактами", "user_mood": "calm|anxious|confused|frustrated|excited"}}"""),
    ("human", """Текущий journey_state: {current_state}
...
async def update_journey_and_summary(
    ...
    try:
        ...
        chain = MEMORY_UPDATE_PROMPT | llm | StrOutputParser()
        result = await chain.ainvoke({
            "current_state": json.dumps(current_state, ensure_ascii=False),
            "current_summary": current_summary,
            "question": question[:500],  # truncate to save tokens
